...
## What Works
- **Shared UI adoption**: The newer abstraction layer is now used in real high-traffic surfaces, not only stored as spare primitives. More menus are unified on `MoreMenuScreen`; repeated state shells use `SignedOutGate`, `AsyncScreenBoundary`, `LoadingState`, and `EmptyState`; profile-style menu rows use `ListRow`; repeated pill selectors use `Chip`; and modal-heavy saved-cart/calendar flows use `ModalHeader`.
...
- **Cart add animations**: In trainer plan shopping, product adds from the grid/detail sheet and bundle adds from bundle detail now animate toward the bottom plan summary using a shared Reanimated overlay; reduced-motion falls back to a footer pulse.
- **Bundle detail in plan shopping**: Plan-shopping bundle detail no longer shows the trainer bottom nav. Trainer plan-shopping bundle taps route to the root bundle screen and that screen now reuses the plan-shopping cart footer while keeping the bundle page’s own back/share chrome.
- **Custom products in shopping step**: Trainer plan shopping now includes a `Custom` tab in the tabbed catalog, so trainer-owned custom products can be searched, created, edited, deleted, and added before review/send. The review screen no longer owns custom-product creation/picking and now keeps only the Add Sessions shortcut. Custom-product create/edit uses image upload via the existing media picker + attachment upload path rather than a manual URL field.
