// Debug the Native Bundle Final Test to see why components aren't showing
...
async function main() {
  // Search for Native Bundle Final Test
  const searchQuery = `
  ...
  // Check if it has bundle components
  ...
  for (const edge of products) {
    ...
    if (product.bundleComponents?.edges?.length > 0) {
      ...
      product.bundleComponents.edges.forEach((comp) => {
        ...
        console.log(`  Option Selections: ${JSON.stringify(comp.node.optionSelections)}`);
      });
    } else {
