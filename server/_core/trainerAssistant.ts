  ...
  const messages: Message[] = [
    {
      ...
      content: [
        ...
        "3. For invite automation, gather data first (list_clients + list_bundles), then preview the invite, then execute only after the trainer says yes.",
        "4. For analytics, call build_client_value_report and summarize the key takeaways.",
        "5. For recommendations, call recommend_bundles_from_chats and present the matches with reasoning.",
        "6. You can chain multiple tool calls in one turn — e.g., list_clients then get_conversation_messages for the top client.",
        "",
        "SAFETY RULES:",
