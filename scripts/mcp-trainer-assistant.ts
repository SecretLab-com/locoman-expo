...
const apiBaseUrl = (
  ...
  .trim()
  .replace(/\/+$/g, "");
type TrainerAssistantMcpAuthContext = {
  authToken?: string;
  impersonateUserId?: string;
};

const authContextStorage =
  new AsyncLocalStorage<TrainerAssistantMcpAuthContext>();

export function runWithTrainerAssistantMcpAuthContext<T>(
  context: TrainerAssistantMcpAuthContext,
  fn: () => Promise<T>,
): Promise<T>;
export function runWithTrainerAssistantMcpAuthContext<T>(
  context: TrainerAssistantMcpAuthContext,
...
function getAuthToken(): string {
  const scopedToken = authContextStorage.getStore()?.authToken;
  if (scopedToken && scopedToken.trim()) return scopedToken.trim();
  return (
    process.env.LOCO_API_TOKEN ||
...
function getImpersonateUserId(): string {
  const scopedImpersonation = authContextStorage.getStore()?.impersonateUserId;
  if (typeof scopedImpersonation === "string" && scopedImpersonation.trim()) {
    return scopedImpersonation.trim();
  }
...
function formatAssistantResponse(result: {
  ...
  if (result.graphData.length > 0) {
    ...
    for (const point of result.graphData) {
      lines.push(
        `- ${point.clientName}: messages=${point.messageCount}, revenue=${point.revenue}`,
      );
    }
  }
...
async function fetchTrainerCoreData() {
  const [profile, clientsRaw, bundlesRaw, ordersRaw, conversationsRaw] =
    await Promise.all([
...
async function buildChatBasedBundleRecommendations(input: {
  ...
  const { profile, clients, bundles, conversations } =
    await fetchTrainerCoreData();
  const trainerId = asCleanString((profile as any)?.id);
  const requestedClientIds = new Set(
    (input.clientIds || []).map((id) => id.trim()).filter(Boolean),
  );
  const requestedBundleIds = new Set(
    (input.bundleIds || []).map((id) => id.trim()).filter(Boolean),
  );
  ...
  const filteredBundles = bundles.filter((bundle) => {
    ...
    if (requestedBundleIds.size > 0 && !requestedBundleIds.has(id))
      return false;
  ...
  for (const conversation of conversations) {
    ...
    const conversationId = asCleanString(
      (conversation as any).conversationId || (conversation as any).id,
    );
    if (
      otherUserId &&
      conversationId &&
      !conversationIdByUserId.has(otherUserId)
    ) {
  ...
  const expectedConversationIds = filteredClients
    .map((client) => {
      ...
      return (
        conversationIdByUserId.get(userId) ||
        toDirectConversationId(trainerId, userId)
      );
    })
    .filter(Boolean);

  const threadsByConversation = await fetchThreadsByConversationIds(
  ...
  for (const client of filteredClients) {
    ...
    if (userId) {
      const conversationId =
        conversationIdByUserId.get(userId) ||
        (trainerId ? toDirectConversationId(trainerId, userId) : "");
      const thread = conversationId
        ? threadsByConversation.get(conversationId) || []
        : [];
  ...
  recommendations.sort(
    (a, b) => b.score - a.score || b.messageCount - a.messageCount,
  );
...
async function buildClientValueReport(input: {
  clientIds?: string[];
  topN: number;
}) {
  const { profile, clients, orders, conversations } =
    await fetchTrainerCoreData();
  const trainerId = asCleanString((profile as any)?.id);
  const requestedClientIds = new Set(
    (input.clientIds || []).map((id) => id.trim()).filter(Boolean),
  ...
  const points = filteredClients.map((client) => {
    ...
    for (const order of orders) {
      ...
      const orderEmail = asCleanString(
      ...
      if (userId && orderClientId && orderClientId === userId) {
        ...
        clientNameLocal &&
        orderName &&
        clientNameLocal === orderName
      ) {
        matched = true;
      }

      if (matched) {
        revenueMinor += toMinor(
          (order as any).totalAmount as string | number | null | undefined,
        );
      }
    }
  ...
  const sortedByMessages = [...points]
    .sort((a, b) => b.messageCount - a.messageCount)
    .slice(0, input.topN);
  const sortedByRevenue = [...points]
...
export function createTrainerAssistantMcpServer(): McpServer {
  ...
  server.registerTool(
    "list_trainers",
    {
      ...
      description:
        "List all trainers on the platform. Coordinator/manager only.",
    ...
    async () => {
      try {
        ...
        return toTextResult(`Returned ${rows.length} trainers.`, {
          trainers: rows,
        });
      } catch (error) {
  ...
  server.registerTool(
    "list_all_users",
    {
      ...
      description:
        "List all users on the platform (any role). Coordinator/manager only.",
      annotations: { readOnlyHint: true },
      inputSchema: {
  ...
  server.registerTool(
    "get_context_snapshot",
    {
      ...
      description:
        "Fetch a snapshot of trainer profile and core counts (clients, bundles, orders, conversations).",
      annotations: { readOnlyHint: true },
      inputSchema: {},
    },
    async () => {
      try {
        const { profile, clients, bundles, orders, conversations } =
          await fetchTrainerCoreData();
        const snapshot = {
          ...
          counts: {
            ...
            publishedBundles: bundles.filter(
              (bundle) =>
                asCleanString(bundle.status).toLowerCase() === "published",
            ).length,
  ...
  server.registerTool(
    "list_clients",
    {
      ...
      inputSchema: {
        status: z
          ...
          .optional()
          .describe(
            "Optional status filter (active, pending, inactive, removed).",
          ),
    ...
    async ({ status, includeNotes }) => {
      try {
        const clients =
          (await trpcClient.clients.list.query()) as unknown as ClientLite[];
        const normalizedStatus = asCleanString(status).toLowerCase();
        const filtered = clients.filter((client) => {
          if (!normalizedStatus) return true;
          return (
            asCleanString(client.status).toLowerCase() === normalizedStatus
          );
        });
        ...
        return toTextResult(`Returned ${rows.length} clients.`, {
          clients: rows,
        });
      } catch (error) {
    ...
    "list_bundles",
    {
      ...
      inputSchema: {
        status: z
          ...
          .optional()
          .describe(
            "Optional status filter (published, draft, pending_review).",
          ),
      },
    },
    async ({ status }) => {
      try {
        const bundles =
          (await trpcClient.bundles.list.query()) as unknown as BundleLite[];
  ...
  server.registerTool(
    "list_conversations",
    {
      ...
      description:
        "List trainer conversation summaries for client chat context.",
      annotations: { readOnlyHint: true },
      inputSchema: {
    ...
    async ({ includeAssistant }) => {
      try {
        const conversations =
          (await trpcClient.messages.conversations.query()) as unknown as Array<
            Record<string, unknown>
          >;
        const rows = conversations
          ...
          .map((conversation) => ({
            conversationId: asCleanString(
              (conversation as any).conversationId || (conversation as any).id,
            ),
            otherUserId:
              asCleanString((conversation as any).otherUserId) || null,
            otherUserName:
              asCleanString((conversation as any).otherUserName) || null,
            otherUserRole:
              asCleanString((conversation as any).otherUserRole) || null,
            lastMessage:
              asCleanString((conversation as any).lastMessageContent) || null,
            updatedAt: asCleanString((conversation as any).updatedAt) || null,
            unreadCount: Number((conversation as any).unreadCount || 0),
          }));

        return toTextResult(`Returned ${rows.length} conversations.`, {
          conversations: rows,
        });
      } catch (error) {
        return toErrorResult(
          `List conversations failed: ${toErrorMessage(error)}`,
        );
      }
    },
  );
    ...
    async ({ conversationId, clientId, limit }) => {
      try {
        ...
        if (!resolvedConversationId && clientId) {
          ...
          const targetClient = (clients as unknown as ClientLite[]).find(
          ...
          if (!clientUserId) {
            return toErrorResult(
              `Client ${clientId} has no linked userId for chat lookup.`,
            );
          }
          ...
          const matchedConversation = (
            ...
            (conversation) =>
              asCleanString((conversation as any).otherUserId) === clientUserId,
          );

          if (matchedConversation) {
            resolvedConversationId = asCleanString(
              (matchedConversation as any).conversationId ||
                (matchedConversation as any).id,
            );
          } else {
            const trainerId = asCleanString((profile as any)?.id);
            resolvedConversationId = trainerId
              ? toDirectConversationId(trainerId, clientUserId)
              : "";
          }
        }
        ...
        return toTextResult(
          `Returned ${rows.length} messages from ${resolvedConversationId}.`,
          {
            conversationId: resolvedConversationId,
            messages: rows,
          },
        ...
        return toErrorResult(
          `Get conversation messages failed: ${toErrorMessage(error)}`,
        );
      }
    },
  );
  ...
  server.registerTool(
    "recommend_bundles_from_chats",
    {
      ...
      description:
        "Deterministically score and recommend best-fit bundles per client based on notes + chat text.",
      annotations: { readOnlyHint: true },
      inputSchema: {
  ...
  server.registerTool(
    "invite_client",
    {
      ...
      description:
        "Send an invitation email to one client. Requires confirm=true.",
      inputSchema: {
    ...
    async ({ email, name, bundleDraftId, message, confirm }) => {
      if (!confirm) {
        return toTextResult(
          "Preview only. Set confirm=true to send this invitation.",
          {
            preview: {
              email,
              name: name || null,
  ...
  server.registerTool(
    "bulk_invite_clients_to_bundle",
    {
      ...
      description:
        "Bulk send invites to multiple clients for one bundle. Requires confirm=true.",
      inputSchema: {
        bundleDraftId: z.string().describe("Target bundle draft ID."),
        clientIds: z
          .array(z.string())
          .optional()
          .describe("Client IDs from trainer_list_clients."),
        message: z.string().max(1000).optional(),
        confirm: z.boolean().default(false),
      },
    },
    async ({ bundleDraftId, clientIds, message, confirm }) => {
      try {
        ...
        if (!confirm) {
          return toTextResult(
            "Preview only. Set confirm=true to send bulk invites.",
            {
              bundleDraftId,
        ...
        return toTextResult(
          `Bulk invite sent for ${invitations.length} clients.`,
          result,
        );
      } catch (error) {
    ...
    async ({
      ...
      try {
        ...
        const grouped = new Map<
          string,
          Array<(typeof recommendationResult.recommendations)[number]>
        >();
        ...
        const preview = {
          ...
          bundleGroups: Array.from(grouped.entries()).map(
            ([bundleDraftId, recs]) => ({
              bundleDraftId,
        ...
        const totalSent = executionResults.reduce(
        ...
        return toErrorResult(
          `Invite from recommendations failed: ${toErrorMessage(error)}`,
        );
      }
    },
  );
  ...
  server.registerTool(
    "client_value_report",
    {
      ...
      description:
        "Build graph-ready client engagement (message count) vs revenue data.",
      annotations: { readOnlyHint: true },
      inputSchema: {
        clientIds: z.array(z.string()).optional(),
    ...
    async ({ clientIds, topN }) => {
      try {
        ...
        const lines = [
          ...
          "",
          "Top by messages:",
          ...report.sortedByMessages
            .slice(0, 5)
            .map(
              (row) =>
                `- ${row.clientName}: ${row.messageCount} msgs, ${row.revenue.toFixed(2)} revenue`,
            ),
          "",
          "Top by revenue:",
          ...report.sortedByRevenue
            .slice(0, 5)
            .map(
              (row) =>
                `- ${row.clientName}: ${row.revenue.toFixed(2)} revenue, ${row.messageCount} msgs`,
            ),
        ];
        ...
        return toErrorResult(
          `Client value report failed: ${toErrorMessage(error)}`,
        );
      }
    },
  );
  ...
  server.registerTool(
    "assistant_preview",
    {
      ...
      description:
        "Preview trainer automation output through the LLM assistant (no mutations).",
      annotations: {
        readOnlyHint: true,
      },
      inputSchema: {
        message: z
          ...
          .min(1)
          .max(4000)
          .describe("Instruction for the trainer assistant."),
  ...
  server.registerTool(
    ...
    async ({ message, provider, conversationId, confirm }) => {
      if (!confirm) {
        return toTextResult(
          "Execution blocked: set confirm=true to allow mutation actions.",
        );
      }
  ...
  server.registerTool(
    "voice_transcribe",
    {
      ...
      inputSchema: {
        audioUrl: z
          ...
          .min(1)
          .describe(
            "Absolute URL (or API-relative URL) for an uploaded audio file.",
          ),
    ...
    async ({ audioUrl, language, prompt }) => {
      try {
        ...
        return toTextResult(
          result.text || "No transcription text returned.",
          result,
        );
      } catch (error) {
...
async function startStdioServer() {
  ...
  console.error(
    `[MCP] Trainer assistant server connected over stdio (${trpcUrl})`,
  );
}
